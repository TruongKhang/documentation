...
"""def load_batch_formatted_from_file(file_name, output_format=DataFormat.TERM_FREQUENCY):
    ...
    if input_format == DataFormat.TERM_SEQUENCE:
        if output_format == DataFormat.TERM_SEQUENCE:
            batch = load_mini_batch_term_sequence_from_sequence_file(fp, num_docs)
        else:
            batch = load_mini_batch_term_frequency_from_sequence_file(fp, num_docs)
    else:
        if output_format == DataFormat.TERM_SEQUENCE:
            batch = load_mini_batch_term_sequence_from_term_frequency_file(fp, num_docs)
        else:
            batch = load_mini_batch_term_frequency_from_term_frequency_file(fp, num_docs)
    fp.close()
    return batch"""
