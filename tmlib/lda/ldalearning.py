...
class LdaLearning(object):
    ...
    def estimate_topic_propotions(self, param_theta):
        raise NotImplementedError("Should have implemented estimate_topic_propotions!")

    def learn_model(self, save_statistic=False, save_model_every=0, compute_sparsity_every=0,
                    save_top_words_every=0, num_top_words=0, model_folder=None, save_topic_propotions=None):
        """
        ...
        if save_topic_propotions is not None:
            self.data.init_database(save_topic_propotions)
        ...
        while not self.data.check_end_of_data():
            ...
            theta = self.estimate_topic_propotions(param_theta)
            if save_topic_propotions is not None:
                self.data.store_topic_propotions(theta)
